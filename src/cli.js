...
function generateRss(context, outputDir) {
  // RSS generation removed — RSS requires absolute site URLs. Reintroduce
  // generation if you want RSS by providing an explicit site URL.
}

function generateSitemap(context, outputDir) {
  // Sitemap generation removed — sitemaps require absolute site URLs. Re-add
  // generation if you want sitemap.xml by providing an explicit site URL.
}
...
async function run() {
  ...
  // RSS and sitemap are not generated by default; re-enable generation with an explicit site URL.
}
